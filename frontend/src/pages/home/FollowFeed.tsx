...
const FollowFeedPage: React.FC = () => {
  ...
  const loadWorks = useCallback(async (page: number) => {
    setLoading(true);
    try {
      const response = await getFollowFeed(page, PAGE_SIZE);
  ...
  return (
    <div style={{ maxWidth: 1060, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
        <HeartOutlined style={{ color: '#667eea', fontSize: 20 }} />
        <Text strong style={{ fontSize: 18, color: '#1a1a2e' }}>关注动态</Text>
      </div>
      ...
      <div ref={sentinelRef} style={{ textAlign: 'center', padding: '24px 0' }}>
        {loading && <Spin />}
        {!hasMore && works.length > 0 && <Text type="secondary" style={{ fontSize: 13 }}>— 已经到底了 —</Text>}
        {!loading && works.length === 0 && <Text type="secondary">暂无关注动态，去关注一些用户吧</Text>}
      </div>
    </div>
  );
};
