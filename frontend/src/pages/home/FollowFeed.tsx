...
const FollowFeedPage: React.FC = () => {
  ...
  const loadWorks = async (page: number) => {
    setLoading(true);
    try {
      const response = await getFollowFeed(page, pageSize);
      setWorks(response.data.data.records);
      setTotal(response.data.data.total);
      setCurrentPage(page);
    } catch (error: any) {
      message.error('加载关注动态失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 1060, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
        ...
        <Typography.Text strong style={{ fontSize: 18, color: '#1a1a2e' }}>关注动态</Typography.Text>
      </div>
      <Spin spinning={loading}>
        {works.length > 0 ? (
          ...
          !loading && <Empty description="暂无关注动态，去关注一些用户吧" />
        )}
      </Spin>
    </div>
  );
