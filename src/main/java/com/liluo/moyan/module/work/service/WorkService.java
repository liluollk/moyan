package com.liluo.moyan.module.work.service;
...
import com.liluo.moyan.module.work.dto.EsSyncMessage;
import com.liluo.moyan.module.work.dto.PublishWorkRequest;
import com.liluo.moyan.module.work.dto.UpdateWorkRequest;
...
import com.liluo.moyan.module.like.entity.UserLike;
import com.liluo.moyan.module.work.entity.Work;
import com.liluo.moyan.module.feed.service.FeedService;
...
import com.liluo.moyan.module.like.mapper.UserLikeMapper;
import com.liluo.moyan.module.user.mapper.UserMapper;
import com.liluo.moyan.module.work.mapper.WorkMapper;
import com.liluo.moyan.common.util.BloomFilterManager;
...
public class WorkService {
    ...
    public WorkVO getWorkDetail(Long workId) {
        log.info("getWorkDetail 开始, workId={}", workId);
        String cacheKey = WORK_CACHE_KEY + workId;

        // 1. 布隆过滤器快速拦截（防止缓存穿透）
        boolean bloomSaysNotExist = bloomFilterManager.isNotExist(String.valueOf(workId));
        log.info("  bloom={}", !bloomSaysNotExist);

        // 2. 第一级缓存：Caffeine本地缓存
        Cache caffeineCache = caffeineCacheManager.getCache("workCache");
        log.info("  caffeineCache={}", caffeineCache != null);
        if (caffeineCache != null) {
            WorkVO cached = caffeineCache.get(cacheKey, WorkVO.class);
            log.info("  caffeineHit={}", cached != null);
            if (cached != null) {
        ...
        log.info("  redisHit={}", redisValue != null);
        if (redisValue != null) {
        ...
        // 4. 缓存未命中，使用互斥锁加载数据
        log.info("  进入loadWithMutexLock");
        WorkVO workVO = loadWithMutexLock(cacheKey, workId, caffeineCache);
        log.info("  loadWithMutexLock返回: {}", workVO != null);
        return fillUserStatus(workVO);
    }
    ...
    private WorkVO loadWithMutexLock(String cacheKey, Long workId, Cache caffeineCache) {
        ...
        if (Boolean.TRUE.equals(lock)) {
            try {
                ...
                log.info("  DB查询结果: {}", work != null ? "找到 workId=" + work.getId() : "NULL!");
                if (work == null) {
    ...
    /**
     * 排行榜：按综合热度排序
     * 热度 = likeCount * 3 + favoriteCount * 2 + commentCount
     */
    public List<WorkVO> getRankingList(int limit) {
        LambdaQueryWrapper<Work> wrapper = new LambdaQueryWrapper<>();
        wrapper.last("ORDER BY (like_count * 3 + favorite_count * 2 + comment_count) DESC LIMIT " + limit);
